...
| ✅ | **Subdomain auto-discovery.** Adding a domain target fires `pg_notify('target_discovery_requested')` → worker hits `crt.sh` (with retry-on-502 backoff) → writes each unique subdomain to `target_discoveries` → user accepts/dismisses on the target detail page. Accepted ones become real targets via the `promote_discovery_to_target` RPC; dismissed ones are hidden permanently. | Matches the mental model: "scan my company". | [`worker/src/strix_worker/discovery.py`](webapp/worker/src/strix_worker/discovery.py), [`migration 014`](webapp/supabase/migrations/20260429000014_target_discoveries.sql), [`components/target/discoveries-panel.tsx`](webapp/frontend/components/target/discoveries-panel.tsx). | M |
