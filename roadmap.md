...
| 🚧 | **Consume `run.test_plan` event** to render a checklist of planned categories before findings exist. The plan checklist now renders on the scan page, including the engine's `summary_text`, per-target planned categories with descriptions, and `dns_only` / `scan_mode` badges. Open: tick off as `check.completed` events arrive (engine doesn't currently emit those — see wishlist P0). | [§3.2](wrapper-wishlist.md#32-runtest_plan-35); [engine PR #35](https://github.com/ClatTribe/strix/pull/35). | [`scans/[id]/page.tsx`](webapp/frontend/app/(app)/scans/[id]/page.tsx) `parseTestPlan`. | S |
...
| 🚧 | **`--preflight` failure UX.** Migration 026 adds `scans.preflight_failed` column; scan-page renders an amber "Target unreachable" banner when set. Worker schema + UI ready, but reliable detection of preflight-vs-generic-failure needs stderr capture to disk + marker check (the heuristic-only-on-empty-events approach mis-fires on any empty-events failure). Worker keeps `preflight_failed=false` until that follow-up lands. | [§1.1](wrapper-wishlist.md#11---preflight-defaults-on-29); [engine PR #29](https://github.com/ClatTribe/strix/pull/29). | [migration 026](webapp/supabase/migrations/20260508000026_dns_only_and_preflight.sql) + scan-page banner. | XS |
