...
| 🚧 | **Webhook integrations (generic + Slack templates).** Per-org Slack incoming-webhook URL stored vault-encrypted via migration 037 + the `worker_decrypt_org_slack_webhook(p_scan_id)` SECURITY DEFINER RPC (mirrors the LLM-key vault pattern; URL prefix re-validated server-side as defence in depth). New `notifier.py` worker module composes a Slack-blocks payload — severity-aware emoji + headline (🚨 critical / ⚠️ high / ✅ clean / 🛑 cancelled / 💸 budget / ❌ failed), severity counts, total cost, run name, and a "View scan" deep-link button when `WRAPPER_ORIGIN` is configured — and POSTs it via httpx with a 5s timeout. Triggered from `run_scan`'s `finally` block so every terminal state notifies. Best-effort everywhere: missing webhook → silent no-op, RPC error → swallowed, HTTP error → logged + continue (never blocks scan finalisation). Settings UI gains a `SlackWebhookSection` with set/clear flow gated to owner+admin via the existing `org.llm_key` RLS pattern. **Open: per-event filtering** (only `finding.created{urgency=fix_now}` etc. — wishlist suggests three event types) — defer until per-user `notification_preferences` lands. **Open: generic webhooks** (non-Slack) — defer until a customer asks. **Open: email channel** — defer until SMTP / SES / Resend is configured. | Plugs into existing incident-response loops. | [migration 037](webapp/supabase/migrations/20260519000037_slack_webhook.sql) + [`notifier.py`](webapp/worker/src/strix_worker/notifier.py) + [API route](webapp/frontend/app/api/orgs/[id]/slack-webhook/route.ts) + `SlackWebhookSection` in [`settings-client.tsx`](webapp/frontend/app/(app)/settings/settings-client.tsx). | M |
...
| 🚧 | **Negative coverage assertions.** "We tested `/api/auth` for SQLi, IDOR, and broken session — clean." Most scanners list only what they found; we should list what we tested *and didn't* find. **Tier-A trust-gap fix shipped (PR #64):** worker now persists the engine's `coverage.json` (required-checks list + actual-ran-checks + gap list) into the new `scans.coverage` JSONB column (migration 039). The scan page renders an amber `CoverageBanner` above the hero strip whenever `status="incomplete"`, with stronger "0-finding ≠ clean bill of health" copy when `findingsCount===0`, plus a small `CoverageMutedPill` decorating the `VendorRiskGauge` so a 100/100 score isn't read as authoritative. **Open: per-target / per-endpoint negative-coverage rendering on the final report** — needs the engine to emit per-target test-plan checkpoints (tools-wishlist §0). | A scan that returns 0 findings looks like the scanner failed; a scan that returns 0 findings *plus a coverage report* looks like a clean bill of health. | [migration 039](webapp/supabase/migrations/20260521000039_scan_coverage.sql) + `_persist_coverage` in [`runner.py`](webapp/worker/src/strix_worker/runner.py) + [`coverage-banner.tsx`](webapp/frontend/components/scan/coverage-banner.tsx) wired into [`scans/[id]/page.tsx`](webapp/frontend/app/(app)/scans/[id]/page.tsx). | M |
...
| ✅ | **`STRIX_FP_AUTO_DISMISS` policy switcher in org settings.** New `organizations.fp_auto_dismiss_policy` column (default `conservative`); admin-gated radio-group section in Settings; worker reads via `_resolve_fp_policy` and forwards as `STRIX_FP_AUTO_DISMISS=<value>` env on every scan. Inline descriptions of when each policy fires. | [§15.1](wrapper-wishlist.md#151-closed-fp-feedback-loop-engine-142); [`engine-usage.md` §4.4](engine-usage.md#44-auto-dismiss-policy-gate). | [migration 027](webapp/supabase/migrations/20260509000027_fp_feedback_loop.sql) + `FpAutoDismissSection` in [`settings-client.tsx`](webapp/frontend/app/(app)/settings/settings-client.tsx). | XS |
...
| ✅ | **`--quiet` / CI mode preset** with copy-pasteable GitHub Actions / GitLab CI snippet generator. New `/scans/ci-snippet` page lets the operator pick platform (GitHub Actions / GitLab CI), scan mode (quick / standard / deep), target, and an optional nightly-cron toggle; renders the matching YAML inline with a copy-to-clipboard button. The snippet pins `ghcr.io/clattribe/strix:stable`, plumbs `LLM_API_KEY` from a CI secret (with explicit instructions per platform on where to set it), and includes the engine's `--quiet` flag (engine PR #121) for parseable pipeline output. Comment-block in each snippet documents the engine's exit-code conventions (0 = clean / 2 = findings / 1 = error) so a CI step's pass/fail is obvious. The new-scan form gains a "CI / CD snippet →" link in the header for power users running strix in their own pipeline (those scans bypass the wrapper). | [§13.3](wrapper-wishlist.md#133-cli--operator-ergonomics-engine-117-121-123-124); [engine PR #121](https://github.com/ClatTribe/strix/pull/121). | New [CI snippet page](webapp/frontend/app/(app)/scans/ci-snippet/page.tsx); link in [new-scan form](webapp/frontend/app/(app)/scans/new/page.tsx). | S |
...
| ✅ | **Cancel button → SIGTERM.** Already wired across migration 012 (`request_scan_cancel` RPC) + `runner.cancel_running_scan` (LISTEN on `scan_cancel` → `proc.terminate()`) + `POST /api/scans/[id]/cancel` + scan-live-view's "Cancel" button + "Cancel pending" pill. Worker also handles negative exit codes (`_exit_code_is_signal` → SIGTERM = -15) so a SIGTERM-killed scan flips to `final_status="cancelled"` with the atomic `finish_scan` in the runner's `finally` block guaranteeing no half-written state. The engine's `run.cancelled` event is streamed verbatim by the events.jsonl tailer — the wrapper consumes the exit signal directly rather than depending on the event arriving, which is the more conservative path. Cross-listed with the Pillar 0 "Scan-cancel button" row that originally landed this work. | [§13.1](wrapper-wishlist.md#131-resilience--cost-gating-engine-112-113-114); [engine PR #114](https://github.com/ClatTribe/strix/pull/114). | S |
...
| 🚧 | **Cost-cap configurator** with per-scan budget. Migration 034 adds `scans.max_cost numeric` + `scans.max_input_tokens integer` and bumps `create_scan_with_targets` to a 13-arg signature with both as optional caps (drops the prior 11-arg overload to dodge PGRST203 ambiguity, same pattern as 026 / 033). New-scan form gains a "Budget cap (optional)" two-input section (USD + token count). API route validates with zod (positive, ≤$10k / ≤1B tokens) and pipes to the RPC. Worker `_build_cmd` appends `--max-cost <usd>` and `--max-input-tokens <n>` only when the values are positive numbers; non-positive / null / missing → no flag → engine's no-cap default. The engine self-exits with code 3 (EXIT_BUDGET_EXCEEDED) when either trips; the worker maps that to `final_status="failed"` with error_message `"scan stopped: budget exceeded"`, and the scan-page failure card detects this distinct path — amber accent (not red), explanatory copy ("planned stop, not a crash" + "raise budget and rerun"), and the "last error from the run" pre-block is suppressed since it'd be misleading. **Open: per-org default budget** with a billing-tier-aware ceiling — deferred until plans are formalised. | [§13.1](wrapper-wishlist.md#131-resilience--cost-gating-engine-112-113-114); [engine PR #113](https://github.com/ClatTribe/strix/pull/113). | [migration 034](webapp/supabase/migrations/20260516000034_cost_cap.sql) + [`runner.py`](webapp/worker/src/strix_worker/runner.py) `_build_cmd` + new-scan form + scan-live-view failure card. | M |
| 🚧 | **HAR / Burp upload UI.** New `ImportsUploader` drag-drop component on the new-scan form: up to 5 files, max 50 MiB each, `.har` (→ `ingest_har_file`) and `.xml` (→ `ingest_burp_file`) only. Browser uploads each file directly to the existing `user-uploads` bucket via the user-context Supabase client at `<org>/scan-imports/<random>/<filename>` — RLS gates the org prefix and the SQL RPC re-validates server-side as defence in depth (migration 035). Refs land on `scans.imports` JSONB; the worker's new `_download_imports` step pulls each file via the service-role client into `<workdir>/imports/<filename>`, with path-traversal + filename-shape + kind-enum re-checks. `_imports_instruction_hint` then composes a directive listing each file by its engine tool name (`ingest_har_file('imports/web.har')`) and appends it to `--instruction` so the agent ingests pre-loaded traffic before its own recon. Per-file failures are logged and dropped — partial pre-load beats failing the whole scan. The redaction notice (sensitive header values stripped at ingest) is mirrored on both the upload form AND the agent instruction so an auditor reviewing either surface sees the same statement. **Open: coverage-uplift summary on `traffic.ingested`** — separate live-pane card showing the engine's reported endpoints/auth-class breakdown post-ingest. Defer until #141's exact event payload shape is sample-data verifiable. | [§15.2](wrapper-wishlist.md#152-har--burp-project-ingestion-engine-141); [engine PR #141](https://github.com/ClatTribe/strix/pull/141). | [migration 035](webapp/supabase/migrations/20260517000035_scan_imports.sql) + `_download_imports` + `_imports_instruction_hint` in [`runner.py`](webapp/worker/src/strix_worker/runner.py) + [API route](webapp/frontend/app/api/scans/route.ts) zod schema + new [ImportsUploader](webapp/frontend/components/scan/imports-uploader.tsx) on the new-scan form. | M |
...
| 🚧 | **Fix-verify targeted rescan.** New "Verify fix" button on every FindingCard whose status isn't `fixed` / `false_positive`. One click POSTs to `/api/findings/[id]/verify-rescan`, which (a) RLS-checks the finding via the user-context client, (b) reads the source scan's `target_id` for inheritance, (c) composes a focused instruction enumerating the original finding's title + CWE/CVE + severity + endpoint and asking for a "no longer present" verdict, (d) calls `create_scan_with_targets` in `quick` mode against the same target, (e) flips the new scan's `verifying_finding_id` (migration 036) via the admin client. The new scan's page renders a cyan banner "Verifying a previously reported finding" with a deep-link to the original. Cross-scan dedup (PR #29) takes care of the rest — if the original fingerprint reappears the new scan emits a finding that links to the same row, otherwise the operator manually triages to `fixed`. **Open: auto-flip-to-fixed** when the verify scan completes cleanly without recurrence — wishlist mentions this; defer until a per-scan "what fingerprints did this run see?" view is in place to drive the comparison. | [§9.3](wrapper-wishlist.md#93-report-ux-post-scan) | [migration 036](webapp/supabase/migrations/20260518000036_verify_rescan.sql) + [API route](webapp/frontend/app/api/findings/[id]/verify-rescan/route.ts) + `VerifyFixButton` in [`finding-card.tsx`](webapp/frontend/components/finding/finding-card.tsx) + scan-page banner. | S |
...
*Catalog source: [`wrapper-wishlist.md`](wrapper-wishlist.md) (engine team's wrapper-facing wishlist) + [`engine-usage.md`](engine-usage.md) (the operating manual). Tiers 0 + 1 + 2 are the **minimum-viable contract**: they close the wrapper-engine integration loop. Everything in Tier 3+ is incremental polish and product expansion.*
