...
| 🚧 | **AI triage with codebase context (RAG).** Stage A.1 shipped: for `local_code` targets, the triage LLM now sees a windowed slice of the actual source around any file/line cited in the finding markdown. Open: Stage A.2 — repository targets via just-in-time shallow clone (needs credential plumbing rework) + Stage B — pgvector similarity for *related* code beyond what Strix cites. | Improves precision from "good guess" to "high confidence". | [`code_context.py`](webapp/worker/src/strix_worker/code_context.py) reads explicit file refs from the report; [`triage.py`](webapp/worker/src/strix_worker/triage.py) injects them into the prompt as ground truth. | L (split: A.1 done, A.2 + B remain) |
