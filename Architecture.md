...
|---|---|---|---|
| 1 | **Fix the severity parser.** `line.split(":", 1)[1]` split *inside* `**Severity:**` and produced `"** high"`, which fails the DB check constraint and gets silently swallowed by the broad `except`. Every finding from a real scan was silently dropped. **Fixed in this PR:** match on the literal `**Severity:**` prefix and strip leftover bold markers. | Findings never appeared in the UI. | [runner.py:238](webapp/worker/src/strix_worker/runner.py:238) |
