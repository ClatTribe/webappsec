...
| 🟠 | **Retry transient LLM 5xx / rate-limits with backoff** | Hit empirically on a real `getedunext.com` scan: Gemini returned `503 ServiceUnavailableError` (*"This model is currently experiencing high demand. Please try again later."*), Strix immediately exited 1, the wrapper marked the whole scan failed. A 30-min scan disappearing because of one transient upstream blip is the worst-feeling failure mode. | Wrapper now surfaces the cause clearly, but a full re-run is the only recovery — the entire 7-min context the agent built is thrown away. | Retry the LLM call 2–3 times with exponential backoff (5s / 15s / 45s) on `429`, `502`, `503`, `504`, and `litellm.ServiceUnavailableError`. Emit `llm.retry_attempted` events so the wrapper can surface "we hit a transient API blip and retried" instead of silently pausing. Only if all retries fail, exit 1. |
